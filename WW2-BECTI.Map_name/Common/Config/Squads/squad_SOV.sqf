...
if(CTI_ECONOMY_LEVEL_WHEELED >= 0) then {
	...
	if(CTI_IFA3LIB_ADDON > 0) then {
		units_to_add = [format["%1R71RusGreen", _sid], 1, 10];
	};
...
if(CTI_ECONOMY_LEVEL_WHEELED >= 1) then {
	if(CTI_IFA3LIB_ADDON > 0) then {
		units_wheeled pushBack [format["%1ifa3_ba64B", _sid], 1, 20];
	};
};
//Level 2
...
if(CTI_ECONOMY_LEVEL_TRACKED >= _level) then {
	...
	if(CTI_IFA3LIB_ADDON > 0) then {
		units_to_add = [format["%1ifa3_t26", _sid], 1, 20];
		if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
			units_to_add = [format["%1ifa3_t26_w", _sid], 1, 20];
		};
		units_to_add pushBack [format["%1ifa3_t60", _sid], 1, 20];
	};
...
if(CTI_ECONOMY_LEVEL_TRACKED >= _level) then {
	...
	if(CTI_IFA3LIB_ADDON > 0) then {
		units_to_add pushBack [format["%1ifa3_t34_76", _sid], 1, 40];
		units_to_add pushBack [format["%1ifa3_t70m", _sid], 1, 40];
		//_c pushBack [format["%1ifa3_pz3J_sov", _sid], 1, 40];
	};
	units_tracked pushBack units_to_add;
	if(CTI_CSA38_ADDON > 0) then {
		units_to_add = [format["%1csa38_t34RU1", _sid], 1, 40];
		if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
			units_to_add = [format["%1csa38_t34RU3", _sid], 1, 40];
		};
	};
	units_tracked pushBack units_to_add;
	_level = _level + 1;
...
if(CTI_ECONOMY_LEVEL_TRACKED >= _level) then {
	...
	units_tracked pushBack units_to_add;
	if(CTI_CSA38_ADDON > 0) then {
		units_to_add = [format["%1csa38_m3a3RU1", _sid], 1, 30];
		units_to_add pushBack [format["%1csa38_matildaii_RU1", _sid], 1, 30];
		if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
			units_to_add = [format["%1csa38_m3a3RU2", _sid], 1, 30];
			units_to_add pushBack [format["%1csa38_matildaii_RU2", _sid], 1, 30];
		};
	};
	units_tracked pushBack units_to_add;
	_level = _level + 1;
...
if(CTI_ECONOMY_LEVEL_TRACKED >= _level) then {
	...
	units_tracked pushBack units_to_add;
	units_to_add = [format["%1LIB_M4A2_SOV", _sid], 1, 30];
	if(CTI_IFA3LIB_ADDON > 0) then {
		units_to_add = [format["%1ifa3_M4A2_SOV", _sid], 1, 30];
		units_to_add pushBack [format["%1ifa3_kv1a", _sid], 1, 40];
		units_to_add pushBack [format["%1ifa3_kv2", _sid], 1, 40];
		units_to_add pushBack [format["%1ifa3_kv8", _sid], 1, 40];
	};
	units_tracked pushBack units_to_add;
	if(CTI_CSA38_ADDON > 0) then {
		units_to_add = [format["%1csa38_valentineMkIIRU1", _sid], 1, 30];
		if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
			units_to_add = [format["%1csa38_valentineMkIIRU2", _sid], 1, 30];
		};
	};
	units_tracked pushBack units_to_add;
	_level = _level + 1;
...
if(CTI_ECONOMY_LEVEL_WHEELED >= 1) then {
	units_antiair pushBack [format["%1LIB_Zis5v_61K", _sid], 1, 30];
};
if(CTI_ECONOMY_LEVEL_WHEELED >= 4) then {
	if(CTI_IFA3_NEW > 0) then {
...
if(CTI_ECONOMY_LEVEL_AIR >= _level) then {
	if(CTI_SAB_ADDON == 1) then {
		units_air pushBack [format["%1sab_la5", _sid], 1, 20];
		_level = _level + 1;
	};
};
//Level 2
if(CTI_ECONOMY_LEVEL_AIR >= _level) then {
	...
	//};
	units_air pushBack [format["%1LIB_Pe2", _sid], 1, 50];
	_level = _level + 1;
};
//Level 3
if(CTI_ECONOMY_LEVEL_AIR >= _level) then {
	...
	if(CTI_SAB_ADDON == 1) then {
		units_to_add pushBack [format["%1sab_p39", _sid], 1, 60];
	};
...
if(CTI_ECONOMY_LEVEL_AIR >= _level) then {
	if(CTI_SAB_ADDON == 1) then {
		units_air pushBack [format["%1sab_il2", _sid], 1, 40];
		_level = _level + 1;
	};
};
