...
if(CTI_ECONOMY_LEVEL_TRACKED >= 2) then {
	...
	if(CTI_IFA3_NEW > 0) then {
		arm_to_add pushBack [format["%1LIB_M3A3_Stuart_UK", _sid], 1, 40];
	...
	if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
		...
		if(CTI_IFA3_NEW > 0) then {
			...
			arm_to_add pushBack [format["%1LIB_M3A3_Stuart_UK_W", _sid], 1, 40];
		};
	};
	if(CTI_CAMO_ACTIVATION == 2 || CTI_CAMO_ACTIVATION == 3) then {		//Desert camo active
		arm_to_add = [[format["%1LIB_Cromwell_Mk4", _sid], 1, 40]];
		if(CTI_IFA3_NEW > 0) then {
			arm_to_add pushBack [format["%1LIB_M3A3_Stuart_DR", _sid], 1, 40];
			arm_to_add pushBack [format["%1LIB_Cromwell_Mk6", _sid], 1, 40];
		};
	};
...
if(CTI_ECONOMY_LEVEL_TRACKED >= 3) then {
	if(CTI_IFA3_NEW > 0) then {
		...
		arm_to_add pushBack [format["%1LIB_M5A1_Stuart_UK", _sid], 1, 40];
	} else {
	...
	if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
		if(CTI_IFA3_NEW > 0) then {
			...
			arm_to_add pushBack [format["%1LIB_M5A1_Stuart_UK_W", _sid], 1, 40];
		};
	};
	if(CTI_CAMO_ACTIVATION == 2 || CTI_CAMO_ACTIVATION == 3) then {		//Desert camo active
		arm_to_add = [[format["%1LIB_UK_DR_M4A3_75", _sid], 1, 40]];
		if(CTI_IFA3_NEW > 0) then {
			arm_to_add pushBack [format["%1LIB_M5A1_Stuart_DR", _sid], 1, 40];
		};
	};
	units_tracked append arm_to_add;
};
