...
_t pushBack "UniversalCarrier [FoW]";
...
if(CTI_ECONOMY_LEVEL_AIR >= _level) then {
	air_to_add = [[format["%1fow_va_f6f_faa", _sid], 1, 60]];
	air_to_add pushBack [format["%1fow_va_f6f_c_faa", _sid], 1, 60];
