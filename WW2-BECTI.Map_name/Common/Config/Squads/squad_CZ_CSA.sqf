...
_t pushBack "TCvz 33 + Ltvz 34 [CSA]";
...
_v pushBack format["ArmoredT%1", _level];
_t pushBack "Ltvz 35 + Pz.2 (CZ) [CSA]";
_p pushBack arm_to_add;
_f pushBack CTI_HEAVY;
_m pushBack 500;
_c pushBack "Armored";
...
if(CTI_ECONOMY_LEVEL_TRACKED >= _level) then {
	...
	if(CTI_CAMO_ACTIVATION == 1 || CTI_CAMO_ACTIVATION == 3) then {		//Winter camo active
		arm_to_add pushback [format["%1csa38_t34cz5", _sid], 1, 50];
	};
	arm_to_add pushback [format["%1CSA38_pzIVvcz", _sid], 1, 50];
