...
CTI_UI_Gear_ReplaceWeapon = {
	...
	if (_changed) then { [_item, _index, [_idc+1, _idc+2, _idc+3]] call CTI_UI_Gear_CheckAccessories }; //--- Do we keep the accessories?
};
...
CTI_UI_Gear_CheckAccessories = {
	...
	_defaults = ["\A3\Ui_f\data\GUI\Rsc\RscDisplayGear\ui_gear_muzzle_gs.paa","\A3\Ui_f\data\GUI\Rsc\RscDisplayGear\ui_gear_side_gs.paa","\A3\Ui_f\data\GUI\Rsc\RscDisplayGear\ui_gear_top_gs.paa"];
	_defaults_process = [];
	...
	if (_item == "" || count(((_gear select 0) select _index) select 1) == 0) then {
		...
		_config_properties = ["MuzzleSlot","PointerSlot","CowsSlot"];
...
CTI_UI_Gear_InitializeProfileTemplates = {
	...
	if (CTI_Log_Level >= CTI_Log_Debug) then {
		["VIOC-DEBUG", "File: Client\Events\Functions_UI_GearMenu.sqf", format["Client saves template: <%1>", _templates]] call CTI_CO_FNC_Log;
	};
	{
...
CTI_UI_Gear_RemoveProfileTemplate = {
	...
	_templates = profileNamespace getVariable format["CTI_VIOWW2_PERSISTENT_GEAR_TEMPLATE_%1", CTI_P_SideJoined];
	_index = -1;
